...
export default function App() {
  ...
  return <><header className="site-header"><a className="brand" href="#compare" onClick={() => setView("compare")}><span className="brand-mark">W</span><span>-Where'sWaldo</span></a><nav><button className={view === "compare" ? "active" : ""} onClick={() => setView("compare")}>Compare</button><button className={view === "dataset" ? "active" : ""} onClick={() => setView("dataset")}>Dataset Demo</button><button className={view === "about" ? "active" : ""} onClick={() => setView("about")}>About</button></nav></header><main>{view === "compare" && <><section className="hero"><span className="eyebrow">Image intelligence / 01</span><h1>Spot what changed.</h1><p>Compare two images side by side and get a clear visual map of every meaningful difference.</p></section><ImageUploadForm onCompare={handleCompare} busy={busy} />{error && <p className="error-message page-error" role="alert">{error}</p>}{result && <ImageAnalysisResult result={result} image1={sources.image1} image2={sources.image2} />}</>}{view === "dataset" && <><section className="hero compact"><span className="eyebrow">Developer tools / 02</span><h1>Explore the reference set.</h1><p>Inspect individual Spot-the-Diff samples without copying the dataset into the application.</p></section><DatasetDemo /></>}{view === "about" && <section className="about-section"><span className="eyebrow">About / 03</span><h1>A modular difference detector.</h1><p>The current comparator uses a transparent pixel-based baseline with alignment, noise reduction, and highlighted output. Its interface is deliberately isolated so a trained model can replace it later.</p><div className="about-grid"><article><span>01</span><h2>Private by default</h2><p>Uploaded images are processed in memory and are not persisted by the comparison service.</p></article><article><span>02</span><h2>External dataset</h2><p>The Lancelot53/spot-the-diff dataset is requested from Hugging Face only when the demo is used.</p></article><article><span>03</span><h2>Ready for ML</h2><p>The comparator contract can later route to a model trained or evaluated outside the request path.</p></article></div></section>}</main></>;
}
